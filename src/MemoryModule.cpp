...
FARPROC MemoryModule::MemoryGetProcAddress(HMEMORYMODULE module, LPCSTR name) {
	...
	printf("\nmodule %p ", module);
	printf("\ncodeBase %d ", ((PMEMORYMODULE)module)->codeBase);
	printf("\ndirectory->Size  %d ", directory->Size);
	printf("\ndirectory->VirtualAddress %d ",  directory->VirtualAddress);
	printf("\nexports  %d ", exports->NumberOfNames);
